...
MASTER_JSON_PATH = os.path.join(PROJECT_ROOT, "master_data.json")

def get_available_desktop_files():
    """Find all available 018 PDF or Excel files on Desktop and samples directory"""
    search_dirs = [
        os.path.join(PROJECT_ROOT, "samples"),
        os.path.join(API_DIR, "samples"),
        "/var/task/samples",
        PROJECT_ROOT,
...
class handler(BaseHTTPRequestHandler):
    ...
    def get_normalized_path(self):
        req_path = self.path
        for hk in ['x-matched-path', 'x-forwarded-uri', 'x-original-uri', 'x-rewrite-url']:
            hval = self.headers.get(hk)
            if hval:
                req_path = hval
                break
    ...
    def do_GET(self):
        ...
        if path.endswith('/status') or path == '/api/status':
        ...
        elif path.endswith('/desktop-samples') or path == '/api/desktop-samples':
            self.send_json_response({
                "files": get_available_desktop_files()
            })
            return

        elif path.endswith('/master-data') or path == '/api/master-data':
            data = load_master_data()
            self.send_json_response(data)
            return

        elif path.endswith('/load-desktop-sample') or path == '/api/load-desktop-sample':
    ...
    def do_POST(self):
        ...
        if path in ["/api/parse-pdf", "/api/parse-file"] or path.endswith('/parse-pdf') or path.endswith('/parse-file'):
        ...
        elif path == "/api/export-excel" or path.endswith('/export-excel'):
            body = self.rfile.read(content_length)
            try:
        ...
        elif path == "/api/save-master-data" or path.endswith('/save-master-data'):
            body = self.rfile.read(content_length)
            try:
